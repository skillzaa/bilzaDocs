...
export default class SpriteSheet extends BaseComponent {
    ...
    draw(p) {
        this.preDraw(p);
        p.drawImageSrcDest(this.img, (this.leftExtraPix + (this.column.value() * this.sourceWidth)), (this.topExtraPix + (this.row.value() * this.sourceHeight)), this.sourceWidth, this.sourceHeight, this.xAligned(), this.yAligned(), this.contentWidth(), this.contentHeight());
        this.postDraw(p);
        return true;
    }
